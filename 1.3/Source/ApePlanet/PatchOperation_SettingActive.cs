...
namespace ApePlanet
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = ApePlanetMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!ApePlanetMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (ApePlanetMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
